...
export class OrganizerService {
  ...
  getAnalytics(userId: string) {
    const totalEvents = this.db.get<any>('SELECT COUNT(*) as c FROM Event WHERE createdById = ?', [userId])?.c || 0;
    const totalAttendees = this.db.get<any>(`
      SELECT COUNT(*) as c FROM Ticket t JOIN Event e ON e.id = t.eventId WHERE e.createdById = ?
    `, [userId])?.c || 0;
    const totalRevenue = this.db.get<any>(`
